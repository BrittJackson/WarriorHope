...
describe("GET /userAccount not logged in", () => {
    test("Not logged in",  function (done) {
        testSession.get('/userAccount')
          .expect(302)
          .end(done)
    });
});
...
describe("GET /userAccount logged in", () => {
    ...
    test("Logged in", function (done) {
        authenticatedSession.get('/userAccount')
        .expect(200)
        .end(done)
    });
});
...
describe("POST /userAccount", () => {
    ...
    // 
    it('Select Change My Password', function(done) {
        testSession.post('/userAccount')
    ...
    // 
    it('Select Change My Email', function(done) {
        testSession.post('/userAccount')
        .set('Cookie', `usernameCookie=${testUser.username}`)
        .send({
            action: "Change My Email"
        })
        .expect(302)
        .end(done);
    });
            ...
            // 
    it('Select Retake Assessment', function(done) {
        testSession.post('/userAccount')
        .set('Cookie', `usernameCookie=${testUser.username}`)
        .send({
    ...
    // 
    it('Select Delete', function(done) {
        testSession.post('/userAccount')
        .set('Cookie', `usernameCookie=${testUser.username}`)
        .send({
            action: "Delete"
